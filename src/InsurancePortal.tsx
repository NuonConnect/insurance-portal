...
// NEXTCARE Benefits Template (Standard benefits for all NEXTCARE networks)
const NEXTCARE_BENEFITS: PlanBenefits = {
  areaOfCover: 'Worldwide',
  annualLimit: 'AED 1 Million',
  network: 'NEXTCARE Network (GP referral required prior to Specialist access)',
  ...
  optical: { enabled: false, value: 'Not Covered / Emergency cases only' },
  ...
  outpatient: 'NEXTCARE Network - OP Restricted to Clinics',
        ...
        Object.keys(plans).forEach(planName => {
          ...
          if (ageBand) {
            ...
            if (rateData && rateData[genderKey as 'M' | 'F']) {
              ...
              else if (provider.includes('WATANIA') && !provider.includes('MEDNET')) network = 'NAS/Mednet TPA';
              ...
              // Get clean provider name (remove _MEDNET and _NEXTCARE suffix)
              let cleanProviderName = provider.replace('_MEDNET', '').replace('_NEXTCARE', '').replace(/_/g, ' ');
