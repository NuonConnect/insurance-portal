        ...
        Object.keys(plans).forEach(planName => {
          ...
          if (ageBand) {
            ...
            if (rateData && rateData[genderKey as 'M' | 'F']) {
              ...
              else if (provider.includes('ORIENT') && !provider.includes('MEDNET')) network = 'Orient/Nextcare';
              else if (provider === 'TAKAFUL_EMARAT') network = 'NEXTCARE';
              
              // Get clean provider name (remove _MEDNET suffix)
              let cleanProviderName = provider.replace('_MEDNET', '').replace(/_/g, ' ');
